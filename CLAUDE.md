...
- **v0.2 (in Arbeit)** — Save-Inspector: read-only Öffnen von `.save`-Dateien mit Screenshot, JSON-Metadaten und Store-Variablen als Tabelle mit Filter. Eigenes `SaveWindow` (ChromeWindow), aufrufbar aus der Toolbar oder per Drag&Drop ins Save-Fenster. Editieren kommt in v0.3. Läuft gegen echte Ren'Py-8.4-Saves (verifiziert an *Sophia Parker Hidden Desires Part 2* v0.230, 417 Variablen).
- **Tests grün (15):** die 10 Archiv-Tests aus v0.1 plus 5 Save-Tests (Metadaten/Screenshot/Store aus einem Python-erzeugten Save; Ren'Py-8-Flat-Format mit `store.foo`-Keys und unbekannten Fake-Klassen via Catch-all; RevertableDict/-List; kaputter Log wirft nicht sondern zeigt Metadaten trotzdem; fehlendes `json` fällt sauber auf leere Metadaten). Interop-Tests überspringen sauber ohne `python3`.
